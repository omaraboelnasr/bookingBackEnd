...
const createRoom = async (req, res, next) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'room add successfully', data: newRoom })
    } catch (err) {
...
const updateRoom = async (req,res,next)=>{
    ...
    let  RoomId  = req.params.roomID
    try{
            let updatedRoom = await roomsModel.findByIdAndUpdate(RoomId,body,{new: true, runValidator: true})
...
const deleteRoom = async (req,res,next)=>{
    ...
    try{
            ...
            res.status(200).json({ message: "Hotel deleted Successfully", data: deletedRoom });
    }catch(err){
        res.status(500).json(err.message)
    }
}
